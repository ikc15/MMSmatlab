...
%% Check if jet is Alfvenic: !!!!!!!!!need to account for negative jets as well!!!!!!!!! 
% indices bracketing exhaust interval and cavity interval
...
maxvijet1= max(vijet(:,1)); % taking the maximum of the exhaust vL 
...
vijet = maxvijet1 - viLin; % speed of jet relative to inflow
...
Tiexh = mean(1/3*Ti(ixTi(1):ixTi(3),1) + 2/3*Ti(ixTi(1):ixTi(3),2));
...
Tiexhpar = mean(Ti(ixTi(1):ixTi(3),1));
Ticavpar = mean(Ti(ixTi(3):ixTi(4),1));
Tiexhperp = mean(Ti(ixTi(1):ixTi(3),2));
Ticavperp = mean(Ti(ixTi(3):ixTi(4),2));
